...
export default function PickupCodeRequestsPage() {
  ...
  async function load() {
    try {
      ...
      setError("");
      setInfo("");
      const data = await pickupCodeRequestsService.list({ status });
  ...
  async function updateRequest(id, nextStatus) {
    try {
      setActionId(id);
      await pickupCodeRequestsService.update(id, { status: nextStatus });
      await load();
    } catch (e) {
  ...
  async function resendAndResolve(item) {
    ...
    try {
      ...
      setError("");
      setInfo("");
      const result = await pickupCodeRequestsService.resendPickupCode(item.preorderId, {
        phone: destination,
      });
      ...
      await pickupCodeRequestsService.update(item.id, {
        status: "RESOLVED",
        reviewNote: `Code de retrait renvoyé par ${result?.channel || "SMS/email"} au ${destination}.`,
      });
      setInfo(`Code de retrait renvoyé au ${destination}. Demande marquée comme traitée.`);
      await load();
    } catch (e) {
      setError(
        e?.response?.data?.message ||
          e?.message ||
          "Impossible de renvoyer le code de retrait.",
      );
    } finally {
      setActionId("");
    }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-950">Demandes de code de retrait</h1>
          <p className="mt-1 text-sm text-gray-500">
            Demandes envoyées depuis l'aide FBO lorsque le client n'a pas reçu son code.
          </p>
        </div>
        <button
          type="button"
                  ...
                  <tr key={item.id} className="align-top">
                    <td className="px-4 py-3">
                      ...
                      <div className="mt-1 text-xs text-gray-500">
                        Colis : {item.preorder?.parcelNumber || "-"}
                      </div>
                    </td>
                    <td className="px-4 py-3">
