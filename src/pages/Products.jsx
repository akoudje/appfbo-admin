...
import { list, create, update, remove, uploadImage } from "../services/productsService";
...
// ⚠️ Assure-toi que ces valeurs existent exactement dans schema.prisma
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Prix de base</div>
              ...
              {product.imageUrl && (
                <div>
                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">URL de l'image</div>
                  <a
...
export default function Products() {
  ...
  const load = async (opts = {}) => {
    ...
    try {
      ...
      const params = {
        ...
        inStock:
          stockValue === "in" ? "true" : stockValue === "out" ? "false" : undefined,
        take: 500,
      };
  ...
  if (!debouncedLoadRef.current) {
    debouncedLoadRef.current = debounce((nextQ, nextActif, nextCategory, nextStock) => {
      load({ q: nextQ, actif: nextActif, category: nextCategory, stock: nextStock });
    }, 350);
  }
  ...
  const handleCreateSubmit = async (payload) => {
    try {
      ...
      // payload doit contenir: sku, nom, prixBaseFcfa, cc, poidsKg, actif, imageUrl?, category?, details?, stockQty?
  ...
  const handleEditSubmit = async (payload) => {
    ...
    try {
      ...
      setRows((prev) => prev.map((x) => (x.id === editProduct.id ? { ...x, ...updated } : x)));

      setDetailsProduct((prev) => (prev?.id === editProduct.id ? { ...prev, ...updated } : prev));
  ...
  const onUploadInline = async (productId, file) => {
    ...
    try {
      ...
      setEditProduct((prev) => (prev?.id === productId ? { ...prev, ...updated } : prev));
  ...
  const onUploadFromModal = async (file) => {
    ...
    setRows((prev) => prev.map((x) => (x.id === editProduct.id ? { ...x, ...updated } : x)));
    setDetailsProduct((prev) => (prev?.id === editProduct.id ? { ...prev, ...updated } : prev));
    setEditProduct((prev) => (prev?.id === editProduct.id ? { ...prev, ...updated } : prev));
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <CreateProductModal
          ...
          loading={modalBusy}
          // si ton modal supporte ces props, sinon ignore-les côté composant
          categoryOptions={PRODUCT_CATEGORIES.filter((x) => x.value)}
        />
          ...
          initialValues={
            editProduct
              ? {
                  ...
                  category: editProduct.category || "NON_CLASSE",
