...
export default function OrderBillingTab({
  ...
  const whatsappContainsPaymentLink =
    Boolean(resolvedPaymentLink) &&
    String(order?.whatsappMessage || "").includes(resolvedPaymentLink);

  const showWaveActions = canUseWave && !isPaymentSucceeded;
    ...
    if (canUseWave) {
      return (
        <Alert tone="blue" title="Paiement Wave / Mobile Money">
          ...
          <p className="mt-1">
            Après facturation, le lien Wave est généré côté backend puis injecté
            dans le message WhatsApp envoyé au client.
          </p>
        </Alert>
      );
    }
  ...
  return (
    <div className="space-y-6">
        ...
        {!canUseWave ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              />
              <Row
                label="Lien dans message"
                value={
                  resolvedPaymentLink
                    ? whatsappContainsPaymentLink
                      ? "✅ Oui"
                      : "⚠️ Non"
                    : "—"
                }
                highlight={Boolean(resolvedPaymentLink && whatsappContainsPaymentLink)}
              />
              <Row
            ...
            {!showWaveActions && isPaymentSucceeded && (
              <Alert tone="emerald" title="Actions verrouillées">
                Le paiement est déjà confirmé. Les actions Wave ont été désactivées
                pour éviter une réinitialisation inutile.
              </Alert>
            )}

            {showWaveDevTools && typeof onSimulateWave === "function" && showWaveActions && (
      ...
      <InfoSection title="Message WhatsApp">
        ...
        {resolvedPaymentLink && !whatsappContainsPaymentLink && (
          <Alert tone="amber" title="Lien absent du message">
            Le lien Wave actif existe bien, mais il n’a pas été détecté dans le
            message WhatsApp stocké sur la commande.
          </Alert>
        )}
